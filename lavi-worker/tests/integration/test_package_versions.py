...
def test_npm_scraper():
    ...
    test_subcases = [
        ...
        ("npm", "lodash", "> 4.17.10, <= 4.17.13", ["4.17.11", "4.17.12", "4.17.13"]),  # Valid query
        ("npm", "lodash", ">= 4.17.10, < 4.17.13", ["4.17.10", "4.17.11", "4.17.12"]),  # Valid query
    ...
    for repo_name, pkg_name, vers, expected in test_subcases:
        if len(expected) != len(set(expected)):
            raise Exception(f"Test bad - duplicates in {repo_name, pkg_name, vers, expected}")
